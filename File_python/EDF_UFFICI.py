...
MURI_bassi=STRUCT([muro,muri_bassi])
wall_1=PROD([muri,Q(34)])
wall_2=PROD([MURI_bassi,Q(4.5)])
...
EV_no=sorted([0,1,7,5,10,2,3,9,8,11,4,6])
...
EV_fini= [20,4,5,18,13,9,7,6,17,19,16,2,3,8]
...
#muri fini e muri spessi
EV_no=sorted([1,34,14,22,37,7,30,19,25,17,15,16,35,26,23])
EV_si= set(range(len(EV))).difference(EV_no)
muri_si=STRUCT(AA(POLYLINE)([[W[EV[e][0]],W[EV[e][1]]] for e in EV_si]))

muri_si=OFFSET([0.15,0.15])(muri_si)
wall_2=PROD([muri_si,Q(3)])
level_2= wall_2
...
#muri fini e muri spessi
EV_no=sorted([30,1,41,8,37,12,19,38,39,21,15,42])
EV_si= set(range(len(EV))).difference(EV_no)
muri_si=STRUCT(AA(POLYLINE)([[W[EV[e][0]],W[EV[e][1]]] for e in EV_si]))
...
#righe circolari
muro_circ1=STRUCT(AA(POLYLINE)([[W[EV[e][0]],W[EV[e][1]]] for e in EV_si]))
righe_circ = T([1,2])([-0.15,-0.15])( OFFSET([0.60,0.60])(muro_circ1))
RIGHE_circ=PROD([righe_circ,INTERVALS(0.20)(1)])
MARMO_circ=T(3)(1.4)(RIGHE_circ)
rip_MARMOcirc=STRUCT(NN(11)([MARMO_circ,T(3)(3)]))

EDF_UFF=STRUCT([rialzo,level1,colonne,rip_MARMO,rip_MARMOcirc])
